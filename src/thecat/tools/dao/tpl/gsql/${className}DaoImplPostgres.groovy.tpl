...
class ${className}DaoImpl extends Dao implements ${className}Dao {
	...
	private ${className} setup${className}FromRow(row, ${entityName}Map=null) {
		...
		def keyRow = row.${entityName?lower_case}_${keyField?lower_case}
...
<#list fieldList as field>
	<#if field.fieldType?starts_with("List") || field.fieldType?starts_with("Set") >
		...
		if (row.${entityName?lower_case}_${subFieldType?lower_case}_${field.fieldId.fieldName?lower_case} != null) {
		...
		<#if field.fieldType?starts_with("Set")>
			${entityName}.${field.fieldName}.put(row.${entityName?lower_case}_${field.fieldName?lower_case}_${field.fieldId.fieldName?lower_case}, ${subFieldType?uncap_first})
		</#if>
		<#list field.fieldList as subField>	
			${subFieldType?uncap_first}.${subField.fieldName} = row.${entityName?lower_case}_${subFieldType?lower_case}_${subField.fieldName?lower_case}
		</#list>
		}
	<#elseif !field.fqFieldType?starts_with("java.lang.") >
		...
		<#list field.fieldList as subField>
			...
			${entityName}.${field.fieldName}.${subField.fieldName} = row.${entityName?lower_case}_${field.fieldType?lower_case}_${subField.fieldName?lower_case}
		</#list>
		}
		
	<#else>
		if (!keyRowExists) {
			if (row.${entityName?lower_case}_${field.fieldName?lower_case} != null) {
				${entityName}.${field.fieldName} = row.${entityName?lower_case}_${field.fieldName?lower_case}
			}
		}
	</#if>
</#list>
...
<#list fieldList as field>
	<#if field.fieldType?starts_with("List") || field.fieldType?starts_with("Set") >
		if (entity.${field.fieldName} != null && entity.${field.fieldName}.${field.fieldId.fieldName} != null) {
			<#assign subFieldType = field.fieldType?substring(field.fieldType?index_of("<") + 1, field.fieldType?index_of(">")) />
			if (sqlCS) {
				result.append("${entityName}.${keyField} <#noparse> ${sqlEqualOp}</#noparse> \'${subFieldType}_${entityName}_${keyField}\' AND")
			} else {
				result.append("UPPER(${entityName}.${keyField}) <#noparse> ${sqlEqualOp}</#noparse> UPPER(\'${subFieldType}_${entityName}_${keyField}\') AND")
			}
		}
	<#elseif !field.fqFieldType?starts_with("java.lang.") >
			if (entity.${field.fieldName} != null && entity.${field.fieldName}.${field.fieldId.fieldName} != null) {
				if (sqlCS) {
					result.append("${entityName}.${field.fieldName}_${field.fieldId.fieldName} <#noparse>${sqlEqualOp} \'${</#noparse>entity.${field.fieldName}.${field.fieldId.fieldName}}\' AND ")
				} else {
					result.append("UPPER(${entityName}.${field.fieldName}_${field.fieldId.fieldName}) <#noparse>${sqlEqualOp} UPPER(\'${</#noparse>entity.${field.fieldName}.${field.fieldId.fieldName}}\') AND ")
				}
			}
	<#else>
			if (entity.${field.fieldName} != null) {
				if (sqlCS) {
					result.append("${entityName}.${field.fieldName} <#noparse>${sqlEqualOp} \'${</#noparse>entity.${field.fieldName}}\' AND ")
				} else {
					result.append("UPPER(${entityName}.${field.fieldName}) <#noparse>${sqlEqualOp} UPPER(\'${</#noparse>entity.${field.fieldName}}\') AND ")
				}
			}
	</#if>
</#list>
	...
	private String generateSqlSelectStatement(${className} ${entityName}, String orderBy, Boolean reverse, Boolean useLike, Boolean useCS,
		int startRow = -1, int pageSize = -1, boolean useCount = false) {
		...
		if (${entityName} != null) {				
			sqlCond = generateSqlSelectWhere(${entityName}, sqlEqualOp, useCS)
			if (!sqlCond.isEmpty()) {
	...
	private List<${className}> _findAll(String orderBy, Boolean desc, int startRow, int pageSize) {
	...
	@Override
	public List<${className}> findAllPaged(int startRow, int pageSize) {
		return _findAll(null, null, startRow, pageSize)
	}

	@Override
	public List<${className}> findAllPagedOrderBy(String orderBy, int startRow, int pageSize) {
		if (orderBy == null) {
	...
	@Override
	public List<${className}> findAllPagedOrderByDesc(String orderBy, int startRow, int pageSize) {
		if (orderBy == null) {
			throw new IllegalArgumentException('The orderBy parameter cannot be null, if sort order is not required use findAllPaged() method instead')
		}
	...
	public ${className} create(${className} entity) {
		...
		try {
			getSession().cacheConnection { connection ->
				...
				begin();
				def insertResult = getSession().firstRow("""
		...
		<#if !field.fieldType?starts_with("List") && !field.fieldType?starts_with("Set") >
			...
			<#else>
				...
				if (insertResult != null) {
					entity.${keyField} = (${keyFieldType}) (insertResult.id)
				}
				result = entity
...
<#if !isAbstract >
	...
	private List<${className}> _findBy(${className} ${entityName}, int startRow, int pageSize) {
		return _findBy(${entityName}, null, null, false, true, startRow, pageSize)
	}

	private List<${className}> _findBy(${className} ${entityName}, String orderBy, Boolean reverse, boolean useLike, boolean caseSensitive, int startRow=-1, int pageSize=-1) {
		if (${entityName} == null) {
	...
	private int _countBy(${className} ${entityName}, boolean useLike) {
		if (${entityName} == null) {
			throw new IllegalArgumentException('The ${entityName} parameter cannot be null')
		}
