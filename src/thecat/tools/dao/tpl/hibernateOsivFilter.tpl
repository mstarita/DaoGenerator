...
/**
 * Hibernate Open Session in View Filter implementation with Extend conversation flag (using session...)
 */
public class HibernateOsivFilter extends Dao implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		try {
			if ((session == null) || (session != null && session.getAttribute(EXTEND_CONVERSATION_FLAG) == null)) {
				//System.out.println("osiv doFilter() - begin hibernate transaction...");
				begin();
			} else {
			...
			if (getSession().getTransaction().isActive()) {
				System.out.println("osiv doFilter() - trying to flush hibernate session...");
				getSession().flush();
			}
			
			if ((session == null) || (session != null && session.getAttribute(EXTEND_CONVERSATION_FLAG) == null)) {
				if (getSession().getTransaction().isActive()) {
					//System.out.println("osiv doFilter() - trying commit hibernate transaction...");
					commit();
