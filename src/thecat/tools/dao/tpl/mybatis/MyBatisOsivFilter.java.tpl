...
/**
 * myBatis Open Session in View Filter implementation with Extend conversation flag (using session...)
 */
public class MyBatisOsivFilter extends Dao implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		try {
			if ((session == null) || (session != null && session.getAttribute(EXTEND_CONVERSATION_FLAG) == null)) {
				//System.out.println("osiv doFilter() - begin myBatis transaction...");
				getSession();
			} else {
			...
			if ((session == null) || (session != null && session.getAttribute(EXTEND_CONVERSATION_FLAG) == null)) {
				if (getSession().getTransaction().isActive()) {
					//System.out.println("osiv doFilter() - trying commit myBatis transaction...");
					commit();
